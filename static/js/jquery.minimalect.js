...
;(function ( $, window, document, undefined ) {
	...
	defaults = {
		...
		class_active: "open", // applied to wrapper when the dropdown is displayed
		...
		class_hidden: "latent", // an item that doesn't match the filter search term
	...
	Plugin.prototype = {
		...
		_init: function() {
			...
			var op = this.options,
				m = this,
                DefaultPlaceholder = m.element.data('placeholder');
			...
			this.input = $(
				'<span class="input"'+
				(op.searchable ? 'contenteditable="true" spellcheck="false"' : '') +
				...
				(this.element.find("option[selected]").html() || DefaultPlaceholder)+
				'</span>'
			).appendTo(this.wrapper);

            $('<div class="custom-select__toggle">  <span class="caret"></span> </div>').appendTo(this.wrapper);
			...
			this.ul = $('<ul class="custom-select__list">'+this._parseSelect()+'<li class="'+op.class_empty+'">'+op.empty+'</li></ul>').appendTo(this.wrapper);
            ...
            // Drop down height response

            var sizer = {
                height : function(){
                    return this.settings * 34 + 19
                },
                settings: this.element.data('size')
            };

            function resizer(){
                var visibleEl = m.items.filter(":visible");
                m.ul.css({
                    'height' : function(){
                        var custHeight = visibleEl.length * 34 + 19;
                        if (sizer.height() >= custHeight) {
                            return custHeight + 'px';
                        } else {
                            return sizer.height() + 'px'
                        }
                    }
                })
            }
			...
			this.input.on("focus click", function(e){
                if(this.isContentEditable) {
                    ...
                    resizer()
                    m.ul.customScrollbar('resize', true)

                    m.wrapper.addClass('focus');
		...
		_showChoices: function(cb){
			...
			if (!wr.hasClass(op.class_active)){
				...
				switch(op.transition) {
					...
					this.ul.css({'opacity':"1", 'visibility':'visible'})
					break;
					default:
					this.ul.show();
					break;
			...
			if (wr.hasClass(op.class_active)){
				...
				switch(op.transition) {
					...
					/*wr.children("ul").fadeOut(op.transition_time);*/
                        wr.children("ul").css({'opacity':"0", 'visibility':'hidden'})
					break;
					default:
					wr.children("ul").hide();
					to = 0;
				...
				setTimeout(function(){
					...
					if(m.input.attr("data-placeholder") != op.placeholder) {
						...
						m.input.text(m.element.data('placeholder'));
					}
				}, 130);
